...
export const membersService = {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST205' || error.code === 'PGRST116' || 
          ...
          console.warn('Members table does not exist in Supabase. Please run the SQL script from SUPABASE_SETUP.md');
          return { data: [], error: { message: 'Table does not exist. Please create the members table first.', code: 'TABLE_NOT_FOUND' } };
        }
  ...
  async add(member) {
    try {
      ...
      const supabaseMember = {
        ...
        nationality: member.nationality || 'Egyptian',
        flag_code: member.flagCode || 'eg',
        description: member.description || '',
        full_description: member.fullDescription || '',
        membership_date: member.membershipDate || '',
        is_active: member.isActive !== undefined ? member.isActive : true,
        active_till: member.activeTill || '',
      ...
      if (error) {
        ...
        if (error.code === 'PGRST205' || error.code === 'PGRST116' || 
            ...
            error.message?.includes('does not exist') ||
            error.message?.includes('schema cache')) {
          console.warn('Members table does not exist in Supabase. Member saved locally only.');
          return { data: null, error: { message: 'Table does not exist. Please create the members table first.', code: 'TABLE_NOT_FOUND' } };
        }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST205' || error.code === 'PGRST116' || 
            ...
            error.message?.includes('does not exist') ||
            error.message?.includes('schema cache')) {
          console.warn('Members table does not exist in Supabase. Member updated locally only.');
          return { data: null, error: { message: 'Table does not exist. Please create the members table first.', code: 'TABLE_NOT_FOUND' } };
        }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST205' || error.code === 'PGRST116' || 
            ...
            error.message?.includes('does not exist') ||
            error.message?.includes('schema cache')) {
          console.warn('Members table does not exist in Supabase. Member deleted locally only.');
          return { error: { message: 'Table does not exist. Please create the members table first.', code: 'TABLE_NOT_FOUND' } };
        }
  ...
  mapLocalToSupabase(localMember) {
    return {
      ...
      is_active: localMember.isActive !== undefined ? localMember.isActive : true,
